...
/* 
  Kunal Samant
  1001534662
  Compile: gcc assignment_kxs4662.c
  Run: ./a.out < labspr20.dat
*/
...
void read_input_file()
{
  ...
  for (i=1; i<workingEdges; i++){
    ...
    &&  edgeTab[residualEdges].head==edgeTab[i].head){
      ...
      // printf("%d\n", edgeTab[residualEdges].capacity);
    }
    else
    {
      residualEdges++;
    ...
    // find max_capacity
    if (U < edgeTab[residualEdges].capacity){
      U = edgeTab[residualEdges].capacity;
    }
  }

  delta = find_delta(U); // find the initial delta value
